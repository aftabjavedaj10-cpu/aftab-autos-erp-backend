...
const CustomerFormPage: React.FC<CustomerFormPageProps> = ({ customer, categories, onBack, onSave, onAddCategory }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    openingBalance: customer?.openingBalance ? String(customer.openingBalance) : 'Rs. 0.00',
  ...
  const validate = () => {
    ...
    if (!formData.customerCode.trim()) newErrors.customerCode = 'Code is required';
    setErrors(newErrors);
  ...
  const handleAction = (stayOnPage: boolean) => {
    if (!validate()) return;
    const payload: Customer = { ...customer, ...formData, balance: isEdit ? customer?.balance : formData.openingBalance } as Customer;
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium">
            {isEdit ? `Updating info for ${customer?.name}` : 'Fill in the details to add a new customer.'}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <form onSubmit={(e) => e.preventDefault()} className="space-y-8">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden">
              ...
              <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Starting Balance</label>
                  <div className="relative">
                    ...
                    <input 
                      ...
                      value={formData.openingBalance.replace('Rs. ', '')}
                      onChange={(e) => setFormData({...formData, openingBalance: `Rs. ${e.target.value}`})}
                    />
                  </div>
                </div>
                ...
                <div className="relative">
                  <div className="flex items-center justify-between mb-2">
                    <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300">Customer Category</label>
                    <button 
                      type="button" 
        ...
        <div className="lg:col-span-1">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden sticky top-32">
            <div className="p-6 border-b border-slate-50 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30">
              <h3 className="font-bold text-slate-900 dark:text-white flex items-center gap-2 uppercase text-xs tracking-widest">
                <span className="text-orange-500">📸</span> Profile Image
              </h3>
            </div>
            <div className="p-8">
