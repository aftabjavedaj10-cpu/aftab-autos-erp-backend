...
import type { Company, Product, PurchaseReturn, SalesInvoiceItem, Vendor } from "../types";
...
interface PurchaseReturnFormPageProps {
  ...
  onNavigate?: (invoice: PurchaseReturn) => void;
  onNavigateNew?: () => void;
  formTitleNew?: string;
...
const PurchaseReturnFormPage: React.FC<PurchaseReturnFormPageProps> = ({
  invoice,
  forceNewMode = false,
  invoices,
  products,
  ...
  onNavigate,
  onNavigateNew,
  formTitleNew = "New Purchase Return",
  formTitleEdit = "Edit Purchase Return",
}) => {
  const idPrefix = "PR";
  ...
  const handleSubmit = (status: string, stayOnPage: boolean = false) => {
    ...
    const invoiceData: PurchaseReturn = {
      ...formData,
      status: finalStatus,
  ...
  const handleVoid = () => {
    ...
    const confirmed = window.confirm(
      `Void purchase return ${formData.id}? This will reverse stock effect and keep document for audit.`
    );
    if (!confirmed) return;
