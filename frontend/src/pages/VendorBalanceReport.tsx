...
const VendorBalanceReportPage: React.FC<VendorBalanceReportPageProps> = ({
  onBack,
  onOpenVendorLedger,
  vendors,
  purchaseInvoices,
  ...
  const rows = useMemo(() => {
    ...
    vendors.forEach((v) => {
      ...
      if (key && v.id) vendorByName.set(key, String(v.id));
    });
    ...
    return vendors
      .map((vendor) => {
        ...
        const opening = toNumber(vendor.balance);
        ...
        const payments = makePayments
          ...
          .filter((pay) => {
            const byId = vendorId && String(pay.vendorId || "") === vendorId;
            const byName = String(pay.vendorName || "").trim().toLowerCase() === vendorName.trim().toLowerCase();
            const fallbackIdByName = vendorByName.get(String(pay.vendorName || "").trim().toLowerCase()) || "";
            return byId || byName || (vendorId && fallbackIdByName === vendorId);
          })
          .reduce((sum, pay) => sum + toNumber(pay.totalAmount), 0);

        const closing = opening + purchases - returns - payments;
  ...
  }, [vendors, purchaseInvoices, purchaseReturns, makePayments, search, sideFilter, startDate, endDate]);
  ...
  return (
    <div className="animate-in fade-in duration-500 pb-10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 p-3">
          <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Total Purchases</p>
          <p className="text-lg font-black text-orange-600">{totals.purchases.toLocaleString()}</p>
        </div>
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 p-3">
          <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Total Payments</p>
          <p className="text-lg font-black text-emerald-600">{totals.payments.toLocaleString()}</p>
        </div>
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 p-3">
          <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Net Payable (DR)</p>
          ...
          <table className="w-full text-left border-collapse">
                ...
                <tr key={row.id || row.name} className="hover:bg-slate-50 text-[11px] border-b border-slate-200">
                  <td className="px-4 py-1.5">
                    ...
                    <p className="text-[9px] font-bold text-slate-500 uppercase">
                      {row.vendorCode || row.id || "-"}
                    </p>
                  </td>
                  <td className="px-4 py-1.5 text-right font-bold">{row.opening.toLocaleString()}</td>
                  <td className="px-4 py-1.5 text-right font-bold text-orange-600">{row.purchases.toLocaleString()}</td>
                  <td className="px-4 py-1.5 text-right font-bold text-indigo-600">{row.returns.toLocaleString()}</td>
                  <td className="px-4 py-1.5 text-right font-bold text-emerald-600">{row.payments.toLocaleString()}</td>
                  ...
                  <td className="px-4 py-1.5 text-center">
                    <button
                      onClick={() => onOpenVendorLedger?.(row.id)}
              ...
              {rows.length === 0 && (
                <tr>
                  <td colSpan={8} className="px-6 py-10 text-center text-[11px] font-black text-slate-400 uppercase tracking-widest">
                    No vendor balances found for selected filters.
                  </td>
                </tr>
              )}
            </tbody>
            <tfoot>
              <tr className="bg-slate-50/70 border-t-2 border-slate-200 text-[11px] font-black">
                ...
                <td className="px-4 py-2 text-right text-orange-700">{totals.purchases.toLocaleString()}</td>
                <td className="px-4 py-2 text-right text-indigo-700">{totals.returns.toLocaleString()}</td>
                <td className="px-4 py-2 text-right text-emerald-700">{totals.payments.toLocaleString()}</td>
...
export default VendorBalanceReportPage;
